...
class AdminController extends Controller
{
    ...
    public function broadcastPage()
    {
        return Inertia::render('Admin/Broadcast', [
            ...
            'templates' => BroadcastTemplate::latest()->get()
        ]);
    }
    ...
    public function storeTemplate(Request $request)
    {
        ...
        BroadcastTemplate::create($request->all());
    ...
    public function destroyTemplate(BroadcastTemplate $template)
    {
        $template->delete();
        return redirect()->back();
    }
    ...
    public function sendBroadcast(Request $request)
    {
        ...
        foreach ($users as $u) {
            $payload = ['chat_id' => $u->chat_id, 'text' => $request->message, 'parse_mode' => 'HTML'];
            $res = Http::post("https://api.telegram.org/bot{$token}/sendMessage", $payload);
            if ($res->successful()) $count++;
        }

        ActivityLog::log("Broadcast: Sent blast to {$count} users.");
        self::alertOwner("Broadcast Sukses: {$count} User terjangkau.");
        
        return redirect()->back();
    }

    // (Remaining methods logs, products, users, etc. remain unchanged and safe...)
    public function dashboard() { $products = Product::withCount(['digitalAssets as total_stock' => function($query) { $query->where('is_used', false); }])->get(); $recentTransactions = Transaction::with('product')->latest()->take(10)->get(); $stats = [ 'total_revenue' => (float) Transaction::where('status', 'PAID')->sum('amount'), 'total_profit' => (float) (Transaction::where('status', 'PAID')->sum('amount') - Transaction::where('status', 'PAID')->join('products', 'transactions.product_id', '=', 'products.id')->sum('products.cost_price')), 'total_users' => TelegramUser::count(), 'avg_rating' => Rating::avg('stars') ?: 0, 'pending_orders' => Transaction::where('status', 'UNPAID')->count(), 'new_users_today' => TelegramUser::whereDate('created_at', now()->toDateString())->count(), 'aov' => Transaction::where('status', 'PAID')->count() > 0 ? (float) Transaction::where('status', 'PAID')->avg('amount') : 0, ]; return Inertia::render('Admin/Dashboard', ['products' => $products, 'recentTransactions' => $recentTransactions, 'stats' => $stats, 'salesChart' => Transaction::where('status', 'PAID')->where('created_at', '>=', now()->subDays(7))->selectRaw('DATE(created_at) as date, SUM(amount) as total')->groupBy('date')->orderBy('date', 'ASC')->get()]); }
