...
class AdminController extends Controller
{
    ...
    public function storeProduct(Request $request) { $request->validate(['name' => 'required', 'price' => 'required|numeric', 'cost_price' => 'nullable|numeric']); $p = Product::create($request->all()); ActivityLog::log("Created: {$p->name}"); self::alertOwner("Produk Baru: {$p->name}"); return redirect()->back(); }
    public function updateProduct(Request $request, Product $product) { $request->validate(['name' => 'required', 'price' => 'required|numeric', 'cost_price' => 'nullable|numeric']); $product->update($request->all()); ActivityLog::log("Updated: {$product->name}"); self::alertOwner("Produk Update: {$product->name}"); return redirect()->back(); }
    ...
    public function storeVoucher(Request $request) { $request->validate(['code' => 'required|string|unique:vouchers,code', 'type' => 'required|in:fixed,percent', 'value' => 'required|numeric', 'limit' => 'nullable|integer']); Voucher::create($request->all()); return redirect()->back(); }
