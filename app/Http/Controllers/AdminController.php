...
class AdminController extends Controller
{
    ...
    {
        // USER DOSSIER: Eager load relationships for CRM Deep-Dive
        $users = TelegramUser::with(['transactions' => function($q) {
    ...
    public function adjustBalance(Request $request, TelegramUser $user)
    {
        $user->increment('balance', $request->amount);
        ActivityLog::log("Credit: Adjusted balance for @{$user->username} by +{$request->amount}");
        self::alertOwner("Credit Sync: @{$user->username} balance +{$request->amount}");
        return redirect()->back();
    }

    // (Support methods restoration...)
    public function products() { return Inertia::render('Admin/Products', ['products' => Product::with('category')->get(), 'categories' => Category::all()]); }
    public function storeProduct(Request $request) { $p = Product::create($request->all()); return redirect()->back(); }
