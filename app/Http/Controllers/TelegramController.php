...
class TelegramController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        ...
        if (isset($update['message'])) {
            ...
            if (Cache::has($cacheKey)) return response()->json(['status' => 'rate_limit']);
    ...
    protected function handleCallback($chatId, $data, $query)
    {
        switch ($data) {
            ...
            case 'PROMPT_VOUCHER': $this->sendMessage($chatId, "🎟️ <b>KLAIM VOUCHER</b>\n\nKetik: <code>/claim KODE</code>"); break;
            default:
    ...
    protected function sendProductsByCategory($chatId, $catId)
    {
        ...
        foreach ($products as $p) {
            ...
            if ($stock > 0) {
                ...
                $buttons[] = [['text' => "🔹 {$p->name} • Rp " . number_format($p->price) . " ({$stock}{$urgency})", 'callback_data' => "BUY_{$p->id}"]];
            }
        }
    ...
    protected function confirmPurchase($chatId, $productId)
    {
        ...
        $buttons = [];
        if ($user->balance >= $product->price) { $buttons[] = [['text' => "💳 Bayar Pakai Saldo", 'callback_data' => "PAY_BAL_{$product->id}"]]; }
        $buttons[] = [['text' => "📸 Bayar QRIS (Instan)", 'callback_data' => "PAY_QRIS_{$product->id}"]];
        $buttons[] = [['text' => '🎟️ Masukkan Voucher', 'callback_data' => "PROMPT_VOUCHER"]];
    ...
    protected function processOrder($chatId, $productId)
    {
        ...
        if (isset($duitku['qrString'])) {
            ...
            $this->sendMessage($chatId, "✅ <b>ORDER DIBUAT</b>", ['inline_keyboard' => [[['text' => 'Bayar Sekarang', 'url' => $duitku['paymentUrl'] ?? '#']]]]);
        }
    }
    ...
    {
        $user = TelegramUser::where('chat_id', $chatId)->first();
        $this->sendMessage($chatId, "👤 <b>PROFIL ANDA</b>\n💰 Saldo: Rp " . number_format($user->balance) . "\n🔗 Ref: <code>{$user->referral_code}</code>", ['inline_keyboard' => [[['text' => '💳 Top Up', 'callback_data' => 'action_deposit'], ['text' => '⬅️ Kembali', 'callback_data' => 'START_CB']]]]);
    }
    ...
    protected function payWithBalance($chatId, $productId)
    {
        ...
        $this->sendMessage($chatId, "✅ <b>PEMBAYARAN BERHASIL</b>\n🔑 <code>{$asset->data_detail}</code>", ['inline_keyboard' => [[['text' => '⬅️ Kembali', 'callback_data' => 'START_CB']]]]);
    }
    ...
    protected function sendFavorites($chatId)
    {
        ...
        $btns[] = [['text' => '⬅️ Kembali', 'callback_data' => 'START_CB']];
        $this->sendMessage($chatId, $text ?? '⭐ <b>FAVORIT ANDA</b>', ['inline_keyboard' => $btns]);
    }
    ...
    protected function applyVoucher($chatId, $code)
    {
        ...
        if (!$voucher || ($voucher->limit > 0 && $voucher->used >= $voucher->limit)) { $this->sendMessage($chatId, "❌ Voucher tidak valid."); return; }
        $this->sendMessage($chatId, "✅ Voucher <b>{$voucher->code}</b> berhasil diklaim!", ['inline_keyboard' => [[['text' => '🛍️ Belanja Sekarang', 'callback_data' => 'SHOW_CATEGORIES']]]]);
    }

    protected function promptSearch($chatId) { $this->sendMessage($chatId, "🔍 <b>CARI PRODUK</b>\nKetik nama produk yang Anda cari:", ['inline_keyboard' => [[['text' => '⬅️ Kembali', 'callback_data' => 'START_CB']]]]); }
